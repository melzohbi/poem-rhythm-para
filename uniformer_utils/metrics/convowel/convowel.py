...
from uniformer_utils.process_english import get_corv
...
class Convowel(datasets.metric.Metric):
    ...
    def _preprocess(self, predicted_words):
        ...
        for predicted_word in predicted_words:
            ...
            corv_predicted = [get_corv(single_word) if single_word and get_corv(
    ...
    def _compute(
        ...
        for i in range(0, len(predicted_words)):
            ...
            # score = 1 - min(distance / (len(predicted_words[i]) + 0.0005), 1)
        ...
        logger.info(
            f"Sample: predicted word corv is {predicted_words[i]} and original corv is {corv[i]}")
